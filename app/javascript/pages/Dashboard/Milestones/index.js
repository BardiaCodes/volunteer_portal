...
const milestones = [
  {name: 15, hours: 0.5, color: '#358fb2'},
  {name: 25, hours: 1, color: '#5ebbde'},
...
const selectMilestone = hours =>
  R.find (m => hours < m.hours) (milestones) || R.last (milestones);

const milestonePercentage = (hours, milestone) =>
  R.clamp (0, 100, Math.round (hours / milestone.hours * 100));
...
const barStyling = (segment, hours, milestone) => {
  const completedMilestoneCount = R.filter (m => hours >= m.hours) (milestones)
    .length;
  const completedWidth = R.clamp (
    0,
    milestoneBarWidth,
    completedMilestoneCount * milestoneBarChunk
  );

  const inProgessWidth = completedMilestoneCount < milestones.length
    ? R.clamp (
        0,
        milestoneBarWidth,
        milestonePercentage (hours, milestone) / 100 * milestoneBarChunk
      )
    : 0;

  const incompleteWidth = R.clamp (
    0,
    milestoneBarWidth,
    milestoneBarWidth - inProgessWidth - completedWidth
  );
...
const Milestones = _props => {
  ...
  const inProgressBarStyling = barStyling (
    'inprogress',
    hours,
    activeMilestone
  );
  const incompleteBarStyling = barStyling (
    'incomplete',
    hours,
    activeMilestone
  );
      ...
      <div className={s.overallProgress}>
        <div className={s.headerAndBar}>
          <div className={s.personalProgress}>
            <div className={s.personalLabel}>
              <div className={s.personal}>
                {t ('volunteer_portal.admin.tab.user.dashboard.personal')}
              </div>
              <div className={s.label}>
                {t ('volunteer_portal.admin.tab.user.dashboard.totalhours')}
              </div>
            </div>
          ...
          <div className={s.bar} style={{width: milestoneBarWidth}}>
            <div
              className={completedBarStyling.classes}
              style={completedBarStyling.styles}
            />
            <div
              className={inProgressBarStyling.classes}
              style={inProgressBarStyling.styles}
            />
            <div
              className={incompleteBarStyling.classes}
              style={incompleteBarStyling.styles}
            />
          </div>
        </div>
        <div className={s.milestoneMarkers}>
          {milestones.map ((milestone, i) => (
            ...
            >
              <p
                className={milestoneLabelStyling (
                  'label',
                  activeMilestone,
                  hours
                )}
              >{`${t ('volunteer_portal.admin.tab.user.dashboard.milestone1')} ${milestone.name}`}</p>
              <p
                className={milestoneLabelStyling (
                  'hours',
                  activeMilestone,
              ...
              >{`${milestone.hours} ${t ('volunteer_portal.admin.tab.user.dashboard.hours1')}`}</p>
            </div>
          ))}
        </div>
      </div>
